...
void printSizes(torch::Tensor& a) {
  std::cout << a.sizes()[0] << " " << a.sizes()[1] << " " << a.sizes()[2]
            << std::endl;
}

auto multivariateLogProb(torch::Tensor& action_mean, torch::Tensor& covar,
                         torch::Tensor& action) {
  ...
  // printSizes(a);
  auto numerator =
      -0.5 * (diff.transpose(1, 2).matmul(covarInverse.matmul(diff)));
  ...
  // std::cout << b << std::endl;
  printSizes(action_mean);
  std::cout << "action_mean.sizes()[1]:    " << action_mean.sizes()[1]
            << std::endl;
  auto denominator =
      torch::logdet(covar).add(log(2 * M_PI) * action_mean.sizes()[1]);
...
void MemoryNN::merge(MemoryNN& r) {
  std::cout << "States size: " << r.states.size()
            << " Rewards size: " << r.rewards.size() << " " << std::endl;
  ...
  this->logprobs.insert(this->logprobs.end(), r.logprobs.begin(),
                        r.logprobs.end());
  this->rewards.insert(this->rewards.end(), r.rewards.begin(), r.rewards.end());
  this->is_terminals.insert(this->is_terminals.end(), r.is_terminals.begin(),
                            r.is_terminals.end());
  this->is_timeups.insert(this->is_timeups.end(), r.is_timeups.begin(),
                          r.is_timeups.end());
...
auto ActorCritic::act(torch::Tensor state, MemoryNN& MemoryNN) {
  ...
  // std::cout << "AAAAAAAAAAAAAAAAAAAAAAA" << action_mean.item<float>() <<
  // std::endl;
  // TODO: case of multiple action variables in the following line
  torch::Tensor action = torch::normal(action_mean.item<double>(), action_std,
                                       {action_mean.size(0)});
  ...
  // std::cout << "OBS in MEMORY has size: " << MemoryNN.states.size() <<
  // std::endl;
  MemoryNN.actions.push_back(action);
  MemoryNN.logprobs.push_back(log_prob);
  return std::make_tuple(action.detach(), log_prob);
}
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> ActorCritic::evaluate(
  ...
  std::cout << "action_logprobs: " << action_logprobs.grad_fn()->name()
            << std::endl;
  printSizes(action_logprobs);
  std::cout << "action_mean.sizes()[1]: " << action_mean.sizes()[1]
            << std::endl;
  auto dist_entropy = multivariateEntropy(action_mean.sizes()[1], cov_mat);
  // std::cout << "dist_entropy: " << dist_entropy.grad_fn()->name() <<
  // std::endl;
  ...
  //     auto sampleActionMean =
  //     action_mean.index({sample}).reshape({action_mean.sizes()[1],
  //     action.sizes()[2]?action.sizes()[2]:1});;

  //     // Eigen::Matrix<double, Eigen::Dynamic, 1> eigen_mean =
  //     tensorToVector(sampleActionMean); auto sampleCovar =
  //     cov_mat.index({sample});

  //     // Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> eigen_covar =
  //     tensorToMatrix(sampleCovar);
  //     // Eigen::EigenMultivariateNormal<double> normalSolver(eigen_mean,
  //     eigen_covar,true,distr_NN(eng_NN));
  //     // auto squeezedAction = torch::squeeze(action);
  //     // PRINT_SIZES(squeezedAction.sizes());
  //     // std::cout << "Action sizes" << " " << action.sizes()[1] << " " <<
  //     action.sizes()[2] << std::endl; auto sampleAction =
  //     action.index({sample}).reshape({action.sizes()[1]?action.sizes()[1]:1,
  //     action.sizes()[2]?action.sizes()[2]:1});

  //     std::cout << "sampleActionMean: " << sampleActionMean.grad_fn()->name()
  //     << std::endl; std::cout << "sampleCovar: " <<
  //     sampleCovar.grad_fn()->name() << std::endl;
  //     // std::cout << "sampleAction: " << sampleAction.grad_fn()->name() <<
  //     std::endl; auto action_logprob = multivariateLogProb(sampleActionMean,
  //     sampleCovar, sampleAction); std::cout << "action_logprob: " <<
  //     action_logprob.grad_fn()->name() << std::endl;
  //     // std::cout << "Evaluation action_logprob " << action_logprob <<
  //     std::endl;
  //     //TODO: make it general
  //     action_logprobs.index({sample}) = action_logprob.squeeze();
  //     auto sample_dist_entropy =
  //     multivariateEntropy(sampleActionMean.sizes()[0], sampleCovar);
  //     dist_entropy.index({sample}) = sample_dist_entropy.squeeze();
  // }

  auto state_value = critic->forward(state);

  return std::make_tuple(action_logprobs, torch::squeeze(state_value),
                         dist_entropy);
  // auto dist_entropy
}
...
void PPO::update(MemoryNN MemoryNN) {
  ...
  for (int index = 0; index < K_epochs; index++) {
    ...
    // std::cout << "Log probs sizes" << logprobs.sizes()[0] << " " <<
    // old_logprobs.sizes()[0] << std::endl;
    ...
    // 0.5*MseLoss->forward(state_values, Rewards) - 0.01*dist_entropy;
...
std::tuple<std::vector<float>, float> getAction(std::vector<float> observation,
  ...
  torch::Tensor observationTensor =
      torch::from_blob(observation.data(), {(long int)observation.size()},
                       torch::TensorOptions().dtype(torch::kFloat32))
          .clone();
  // std::cout << "HERE IS WHAT YOU SHOULD LOOK AT" << observation << '\n'
  // << observation.data() << '\n' << observationTensor << std::endl;
  auto [actionTensor, logProbTensor] =
      ppo.select_action(observationTensor, memoryNN);
  actionTensor = actionTensor.contiguous();

  std::vector<float> actionVec(
      actionTensor.data_ptr<float>(),
      actionTensor.data_ptr<float>() + actionTensor.numel());
