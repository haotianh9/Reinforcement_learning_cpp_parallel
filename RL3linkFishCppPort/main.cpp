...
struct ActorCritic: torch::nn::Module {
    ...
    auto act(torch::Tensor state, Memory memory){
        ...
        memory.states.push_back(state);
        memory.actions.push_back(action);
        memory.logprobs.push_back(log_prob);
...
class PPO {
    ...
    auto select_action(torch::Tensor state, Memory memory){
        //TODO: check
        state = state.reshape({1, -1});
        return policy_old.act(state, memory).cpu().flatten();

    }

    auto update(Memory memory){
        auto memoryRewards = memory.rewards;
        auto memoryIsTerminals = memory.is_terminals;
        auto memoryStates = memory.states;
        std::reverse(memoryRewards.begin(), memoryRewards.end());
        std::reverse(memoryIsTerminals.begin(), memoryIsTerminals.end());
        std::reverse(memoryStates.begin(), memoryStates.end());
        torch::Tensor discounted_reward = torch::tensor({0.0});
        vector<torch::Tensor> rewards;
        for(int index = 0; index < memoryRewards.size(); index++){
            auto reward = memoryRewards[index];
            auto is_terminal = memoryIsTerminals[index];
            auto memoryState = memoryStates[index];
            if(is_terminal){
                auto value = policy.critic->forward(memoryState.squeeze());
        ...
        auto old_states = torch::squeeze(torch::stack(memory.states));
        auto old_actions = torch::squeeze(torch::stack(memory.actions));
        auto old_logprobs = torch::squeeze(torch::stack(memory.logprobs));
